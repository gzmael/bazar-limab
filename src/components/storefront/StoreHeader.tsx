  ...
  return (
    <header className="sticky top-0 z-30 border-b border-border/80 bg-background/90 shadow-sm backdrop-blur-xl supports-[backdrop-filter]:bg-background/75">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-2 px-4 py-3 md:flex-row md:items-center md:justify-between md:gap-4">
        <div className="flex min-h-11 items-center justify-center md:justify-start">
          <Link
            href="/"
        ...
        >
            ...
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.Content
                  align="end"
                  className="z-50 min-w-[12rem] rounded-lg border border-border bg-popover p-1 text-popover-foreground shadow-md"
                  sideOffset={6}
                >
                  {rooms.map((r) => (
          ...
          <Link
            href="/cart"
            className="relative inline-flex min-h-11 min-w-11 items-center justify-center rounded-full text-muted-foreground hover:bg-muted hover:text-foreground"
      ...
      <div
        className="hidden border-t border-sidebar-border bg-sidebar px-4 py-1.5 text-center text-xs text-sidebar-foreground md:block"
        aria-hidden
      >
        Catálogo curado — Bazar Lima Basilio
      </div>
    </header>
  )
}
